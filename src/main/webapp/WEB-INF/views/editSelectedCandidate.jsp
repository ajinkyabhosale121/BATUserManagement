			...
			<div class="menu-sidebar__content js-scrollbar1">
				<nav class="navbar-sidebar">
					<ul class="list-unstyled navbar__list">
						...
						<li><a href="<c:url value='/viewInterviewShedule' />"> <i
								class="far fa-check-square"></i>Interview Shedules
						</a></li>
						<li class="active"><a
							href="<c:url value='/viewSelectedCandidates' />"> <i
								class="fas fa-calendar-alt"></i>Selected Candidates
						</a></li>
...
<%-- 									<div class="card-header">${hmessage}</div> --%>
									<div class="card-body">
										...
										<hr>
										<form:form action="editSelectedCandidate" method="post"
											novalidate="novalidate" modelAttribute="candidate">
											<form:input style="color: #4272d7;" type="hidden" path="" id="inshedules"
												value="${shedules}" />
											<form:input type="hidden" path="selid" id="selid" />
											
											<div class="form-group">
												<label style="color: #00ad5f;" for="request_id" class="control-label mb-1">Request:</label>

												<form:select style="color: #4272d7;" path="request_id" class="form-control"
													onchange="getRequest()">

													<c:forEach items="${requests}" var="request">

														<form:option value="${request.getReqid()}"
															label="Language : ${request.getLanguage()} Experience : ${request.getYears_Of_Experience()} Vacancy : ${request.getVacancies()}" />
											...
											<div class="form-group">
												...
												<form:select style="color: #4272d7;" path="user_id" class="form-control">
											...
											<div class="form-group">
												<label style="color: #00ad5f;" for="interview_id" class="control-label mb-1">Candidate:</label>
												<form:select style="color: #4272d7;" path="interview_id" class="form-control">

													<c:forEach items="${shedules}" var="shedule">

														<form:option value="${shedule.getSheid()}"
															label="${shedule.getName()}" />

													</c:forEach>

												</form:select>
												<form:errors path="interview_id" cssClass="error"
													style="color: red;" />
											</div>

											<div class="form-group">
												<fmt:formatDate pattern="yyyy-MM-dd"
													value="${candidate.joiningDate}" var="dateString" />
												<label style="color: #00ad5f;" for="joiningDate" class="control-label mb-1">Joining
													Date:</label>
												<form:input style="color: #4272d7;" type="date" value="${dateString}"
													path="joiningDate" id="joiningDate" class="form-control" />
											...
											<c:choose>
												...
												<c:when test="${delete}">
													<input type="submit" name="mysub" value="Delete"
														class="btn btn-danger" />
												</c:when>
												<c:otherwise>
													<input type="submit" name="mysub" value="Add"
	...
	<script type="text/javascript">
		...
		function notifyRe(message){
		    ...
		    window.location.href = url + "/viewSelectedCandidates";
		}
