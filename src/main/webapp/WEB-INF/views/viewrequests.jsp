...
<c:url value="/resources/js/tinycolor.js" var="tinycolorjsjs" />
...
<body class="animsition">
			...
			<div class="menu-sidebar__content js-scrollbar1">
				<nav class="navbar-sidebar">
					<ul class="list-unstyled navbar__list">
						...
						<li class="active"><a href="<c:url value='/viewrequests' />">
								<i class="fas fa-table"></i>Recruitment Requests
						</a></li>
				...
				<div class="section__content section__content--p30">
					<div class="row m-t-30">
						<div class="col-md-12">
							...
							<form name="requestReg" action="requestregistration" method="get"
								id="requestReg">
								<!-- DATA TABLE-->
										...
										<tbody>
											<c:forEach items="${requests}" var="request">
												<tr class="reqtr">
													...
													<td class="mytds" style="width: 5%;">

														<div>
															<input type="checkbox" class="fancy-check chkbox"
																name="chkbox" id="chkbox${request.getReqid()}"
																value="${request.getReqid()}" /> <label
																for="chkbox${request.getReqid()}"><span></span>
															</label>
														</div>
													</td>
													<td class="mytds">${request.getLanguage()}</td>
													<td class="mytds">${request.getYears_Of_Experience()}</td>
													<td class="mytds">${request.getJob_Desc()}</td>
													<td class="mytds">${request.getVacancies()}</td>
	...
	<script src="${tinycolorjsjs}"></script>
	...
	var arrayColors = [];
	var trs = document.getElementsByClassName("reqtr");
	for (var i = 0; i < trs.length; i++){
		
		tds = trs[i].getElementsByClassName("mytds");
		
		arrayColors[i] = getDarkRandomColor();
		
		console.log("VR: " + tds[1].innerText + "-" + tds[2].innerText);
		console.log("VR: " + arrayColors[i]);
		
		setCookie(tds[1].innerText + "-" + tds[2].innerText, arrayColors[i], (24*60*60*1000));
		
		for (var j = 0; j < tds.length; j++){
			tds[j].style.color = arrayColors[i];
		}
	}
